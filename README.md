...
同じ入力から潜在空間をサンプリングして10パターン生成。学習が進むにつれて4,5,6の画像をランダムに復元可能となります。

![output fixed 5](fig/output_fixed_5.png)

**数字 8 の生成結果（Epoch 1 → 50 → 244）**

同じ入力から潜在空間をサンプリングして10パターン生成。学習が進むにつれて7,8,9の画像をランダムに復元可能となります。
...
学習が進んでも出力結果が、ノイズ画像のようになりうまく画像生成ができていないことがわかります。  
つまり、入力と教師のペアを変えると学習できないことがわかります。
