...
## 目的
ある数字画像を入れた時、-1, 0, +1した画像を生成するためのVAEの学習方法・損失関数の設定  
一般的に考えられる1対多のVAEの学習では、同じ教師ドメインに対して生成可能であるが、異なるドメインに対しては生成不可能。  
CVAEの考えがあるが、conditionalである条件を入力として与えることなく、多様なドメイン空間を復元可能なVAEを学習したい。
